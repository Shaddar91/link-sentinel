...
def test_repo_depth_config_detailed():
    ...
    assert cfg["agent"] == "code-reviewer"
...
def test_create_repo_task_detailed_uses_code_reviewer():
    repo = _make_repo()
    task = create_repo_analysis_task(
        repo=repo,
        clone_dir=Path("/tmp/clone"),
    ...
    assert "**Target Agent:** code-reviewer" in task
    assert "**Priority:** High" in task
    assert "**Complexity-Score:** 85" in task
    assert '"agent": "code-reviewer"' in task
...
def test_create_video_task_detailed():
    ...
    assert "**Analysis-Depth:** detailed" in task
    assert "**Target Agent:** scholar" in task
    assert "**Priority:** High" in task
...
if __name__ == "__main__":
    tests = [
        ...
        test_create_repo_task_detailed_uses_code_reviewer,
